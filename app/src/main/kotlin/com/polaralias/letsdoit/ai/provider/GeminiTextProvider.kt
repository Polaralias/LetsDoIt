...
import com.polaralias.letsdoit.ai.model.AiParsePayload
import com.polaralias.letsdoit.ai.model.AiParseResult
import com.polaralias.letsdoit.ai.model.AiProject
import com.polaralias.letsdoit.ai.model.AiTask
import com.polaralias.letsdoit.ai.model.PlannedStep
...
@Singleton
class GeminiTextProvider @Inject constructor(
    moshi: Moshi
) : AiTextProvider {
    ...
    override suspend fun parse(input: AiParsePrompt): ProviderResponse {
        ...
        val complexity = if (tasks.isEmpty()) 0.0 else minOf(1.0, tasks.size * 0.04)
        val averageConfidence = if (tasks.isEmpty()) 0.0 else 0.78
        val result = AiParseResult(
            can_handle = averageConfidence >= 0.75 && complexity <= 0.4,
            complexity_score = complexity,
            reason = if (tasks.isEmpty()) "No tasks detected" else "Gemini heuristic extraction",
    ...
    override suspend fun splitSubtasks(title: String, notes: String?): ProviderResponse {
        val values = listOfNotNull(title, notes)
            .joinToString(" ")
            .split('.', '\n', ';')
            .map { it.trim() }
            .filter { it.isNotEmpty() }
        val body = listAdapter.toJson(values)
